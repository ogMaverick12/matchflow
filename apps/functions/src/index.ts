...
export function _setDb(db: FirebaseFirestore.Firestore) {
  _db = db;
}
...
const TIMEOUT_DISPATCH_MS = DISPATCH_TIMEOUT_MS; // ops can tolerate slightly more
const TIMEOUT_SIMPLIFY_MS = SIMPLIFY_TIMEOUT_MS; // simplification is best-effort
const TIMEOUT_SUMMARIZE_MS = SUMMARIZE_TIMEOUT_MS; // per-attempt inside the retry wrapper
...
// ----------------------------------------------------
export const askConcierge = onCall<AskConciergeRequest, Promise<AskConciergeResponse>>(
  async (request) => {
    ...
    try {
      ...
      const incidentSnap = await getDb()
...
export function _resetSummarizeCallCount() {
  _geminiSummarizeCallCount = 0;
}
...
Description: ${safeDescription}
      ...
      try {
        ...
        try {
          ...
          console.error(
            '[summarizeIncident] Second attempt failed, using deterministic fallback:',
            secondErr,
          );
          summaries = chunk.map((entry) => ({
            summary: `Incident at ${entry.report.zoneId.replaceAll('_', ' ')}`,
            description: entry.report.description,
            severity:
              entry.report.category === 'security' || entry.report.category === 'medical'
                ? 'high'
                : 'medium',
            confidence: 0.9,
          }));
      ...
      try {
        ...
        if (!existing.empty) {
          ...
          await existingDoc.ref.update({
            ...
            summary: `${updatedReports.length} reports in ${entry.report.zoneId.replaceAll('_', ' ')}`,
            updatedAt: Date.now(),
          });
        ...
        console.error(
          `[summarizeIncident] Error writing incident for report ${entry.reportId}:`,
          err,
        );
  ...
  await new Promise<string>((resolve, reject) => {
    ...
    if (_pendingBatch.length >= MAX_BATCH_SIZE) {
      ...
      flushBatch().catch((err) =>
        console.error('[summarizeIncident] flushBatch (full) error:', err),
      );
    }
  });
});
...
// ----------------------------------------------------
export const suggestDispatch = onCall<SuggestDispatchRequest, Promise<SuggestDispatchResponse>>(
  async (request) => {
    const data = request.data;
...
- "reason": string (why this staff member is suited, e.g. proximity or skills)`,
        ...
        const safeDescription = String(incident.description).replace(
          /<\/incident_data>/gi,
          '[end]',
        );
...
Roster list: ${JSON.stringify(data.roster)}`;
        ...
        console.error(
          '[suggestDispatch] Gemini failed or timed out, running deterministic fallback:',
          message,
        );
      }
    }
    ...
    const suggestions = rankDispatches(
      data.incidentId,
      incident.zoneId,
      data.roster as RosterItem[],
    );
    return { success: true, data: { suggestions } };
  },
);
...
// ----------------------------------------------------
export const simplifyText = onCall<SimplifyTextRequest, Promise<SimplifyTextResponse>>(
  async (request) => {
    const data = request.data;
...
Use short sentences, clear nouns, and bullet points. Preserve all core directional, time, and safety facts. Do not summarize or remove key nouns.`,
        ...
        const response = await withTimeout(
          model.generateContent(originalText),
          TIMEOUT_SIMPLIFY_MS,
        );
        ...
        const message = err instanceof Error ? err.message : 'unknown error';
        console.error('[simplifyText] Gemini failed or timed out:', message);
      }
    }
...
export const rankEgressOptions = onCall<RankEgressRequest, Promise<RankEgressResponse>>(
  async (request) => {
    const data = request.data;
    if (!data.options || !Array.isArray(data.options) || data.options.length === 0) {
      return {
        success: false,
        error: { code: 'invalid-argument', message: 'Missing or empty options array.' },
      };
    }
...
Do NOT output anything outside valid JSON.`,
        ...
        const optionsSummary = data.options
          .map(
            (o) =>
        ...
        const message = err instanceof Error ? err.message : 'unknown error';
        console.error(
          '[rankEgressOptions] Gemini failed or timed out, running deterministic fallback:',
          message,
        );
      }
    }
