...
import { Send, FileText, Users, HeartHandshake, ShieldAlert, Settings, HelpCircle } from 'lucide-react';
...
export default function VolunteerPage() {
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto' }}>
      {/* §9: h1 page heading */}
      <h1 className="display-title" style={{ fontSize: '28px', fontWeight: 'bold', margin: '0 0 8px 0' }}>
      ...
      <form
        ...
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', margin: '0 0 16px 0', fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        <div role="group" aria-labelledby={categoryGroupId} style={{ marginBottom: '20px' }}>
          <div
            id={categoryGroupId}
            style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: 'bold' }}
            aria-hidden="true">
            CATEGORY
          </div>
            ...
            {categories.map(cat => (
              <button
                ...
                style={{
                  ...
                  backgroundColor: category === cat.id ? 'var(--primary-accent)' : 'var(--bg-surface-elevated)',
          ...
          {reports.map(rep => (
            <li key={rep.id} className="glass-panel" style={{ padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontWeight: 'bold', fontSize: '13px', color: 'var(--primary-accent)', letterSpacing: '0.5px' }}>
                  {rep.category.toUpperCase()}
                </span>
                {/* §9: time element for screen readers */}
                <time
                  dateTime={new Date(rep.timestamp).toISOString()}
                  style={{ fontSize: '11px', color: 'var(--text-secondary)', fontVariantNumeric: 'tabular-nums' }}>
