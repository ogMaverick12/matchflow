...
export default function OpsLayout({
    ...
    return (
      <main style={{
        ...
        <div className="glass-panel" style={{
          ...
          <Shield size={64} color="var(--alert-accent)" aria-hidden="true" style={{ marginBottom: '16px', display: 'inline-block' }} />
          {/* §9: Access denied is effectively a page — needs an h1 */}
          <h1 className="display-title" style={{ fontSize: '28px', fontWeight: 'bold', margin: '0 0 12px 0', color: 'var(--text-primary)' }}>
            ACCESS DENIED
          </h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '15px', lineHeight: '1.5', margin: '0 0 24px 0' }}>
            {reason}
          </p>
  ...
  return (
    <div className={`ops-layout-container ${wrapperClass}`} style={{
      display: 'flex',
      flexDirection: 'row',
      minHeight: '100vh',
      backgroundColor: 'var(--bg-base)',
      ...
      <aside className="ops-sidebar" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '32px' }}>
          ...
          <span style={{ fontWeight: 'bold', fontSize: '18px', letterSpacing: '1px' }}>MATCHFLOW OPS</span>
        </div>
        ...
        <div style={{
          ...
          <span style={{ display: 'block', color: 'var(--text-secondary)', fontSize: '11px', textTransform: 'uppercase' }}>Current Role</span>
          <span style={{ fontWeight: 'bold', color: 'var(--primary-accent)' }}>{role.toUpperCase()}</span>
        </div>

        {/* §9: aria-label required on nav landmark for uniqueness */}
        <nav aria-label="Operations navigation" style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
          {navLinks.map(link => {
      ...
      <main id="ops-main-content" style={{ flex: 1, padding: '24px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        {children}
      </main>
      ...
      <style jsx global>{`
        @media (max-width: 768px) {
          ...
          .ops-sidebar nav, .ops-sidebar button, .ops-sidebar div:nth-child(2) {
