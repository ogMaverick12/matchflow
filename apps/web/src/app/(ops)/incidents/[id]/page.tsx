...
export default function IncidentDetailPage() {
  ...
  useEffect(() => {
    ...
    const unsubDispatches = db.subscribeToDispatches(
      session.role,
      (liveDispatches) => {
  ...
  if (loading) {
    return (
      ...
      >
        <span className="numeric-glow" style={{ fontSize: '15px' }}>
          Loading incident profile...
        </span>
      </div>
    );
  }
      ...
      <div style={{ padding: '24px' }}>
        ...
        >
          ...
          <h4
            ...
            style={{ color: 'var(--alert-accent)', margin: '0 0 8px 0' }}
          >
  ...
  if (!incident) {
    return (
      <div style={{ padding: '40px 16px', textAlign: 'center' }}>
        <h3 className="display-title" style={{ fontSize: '22px', marginBottom: '16px' }}>
          Incident profile not found
        </h3>
        <Link href="/dashboard" style={{ color: 'var(--primary-accent)', fontWeight: 'bold' }}>
          Return to Dashboard
        </Link>
      </div>
    );
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      >
        ...
        >
          ...
          <RouteCard
            destinationName={`${incident.zoneId.replaceAll('_', ' ')} (Level ${incident.level})`}
            totalTimeSeconds={
              incident.severity === 'high' ? 240 : incident.severity === 'medium' ? 150 : 90
            }
            isAccessible={false}
            pathNodesCount={3}
            congestionLevel={
          ...
          <span
            style={{
              ...
              padding: '4px 12px',
              borderRadius: '4px',
              border: isIncidentActive
                ? '1px solid var(--alert-accent)'
                : '1px solid var(--secondary-accent)',
              backgroundColor: isIncidentActive
                ? 'rgba(239, 68, 68, 0.1)'
                : 'rgba(16, 185, 129, 0.1)',
        ...
        >
          <div>
            ...
            <span style={{ fontWeight: 'bold' }}>
              {incident.zoneId.replaceAll('_', ' ')} (Level {incident.level})
            </span>
          </div>
          <div>
            <span
            ...
            <span
              className="numeric-glow green-glow"
              style={{ fontWeight: 'bold', color: 'var(--secondary-accent)' }}
            >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '24px' }}>
              ...
              {dispatches.map((disp) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {disp.suggestedBy === 'ai'
                        ? `Reallocate resource [${disp.staffName}] to ${incident.zoneId.replaceAll('_', ' ')} concourse entrance to control passenger queues.`
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {reports
              .filter((r) => incident.sourceReportIds.includes(r.id))
              .map((rep) => (
                <div
                  key={rep.id}
                  className="glass-panel"
