...
/** All navigable nodes in the Mercedes-Benz Stadium concourse graph. */
export const MERCEDES_BENZ_NODES: ConcourseNode[] = [
  ...
  },
  {
    ...
    zone: 'Zone_A',
    level: '100',
    accessibilityTags: ['elevatorAdjacent', 'rampAccess'],
    x: 40,
    y: 15,
  },
  {
    id: 'concession_burgers',
    name: 'Benz Burgers',
    type: 'concession',
    zone: 'Zone_A',
    level: '100',
  ...
  {
    id: 'seating_101',
    name: 'Seating Section 101',
    type: 'seatingBlock',
    zone: 'Zone_A',
    level: '100',
    accessibilityTags: ['accessibleSeating'],
    x: 45,
    y: 25,
  },
  {
    ...
    zone: 'Zone_A',
    level: '100',
    accessibilityTags: ['elevatorAdjacent'],
    x: 50,
    y: 20,
  },
  ...
  {
    id: 'restroom_103',
    name: 'Restroom 103 (Non-Accessible)',
    type: 'restroom',
    zone: 'Zone_C',
    level: '100',
    accessibilityTags: [],
...
/** Walkable edges connecting concourse nodes, with travel times and accessibility flags. */
export const MERCEDES_BENZ_EDGES: ConcourseEdge[] = [
  ...
  {
    fromNodeId: 'restroom_101',
    toNodeId: 'concession_burgers',
    walkTimeSeconds: 20,
    accessible: true,
  },
  {
    fromNodeId: 'concession_burgers',
    toNodeId: 'seating_101',
    walkTimeSeconds: 25,
    accessible: true,
  },
  ...
  {
    fromNodeId: 'escalator_east',
    toNodeId: 'concession_tacos',
    walkTimeSeconds: 25,
    accessible: true,
  },
  {
    fromNodeId: 'concession_tacos',
    toNodeId: 'seating_110',
    walkTimeSeconds: 30,
    accessible: true,
  },
  ...
  {
    fromNodeId: 'concession_pizza',
    toNodeId: 'seating_120',
    walkTimeSeconds: 20,
    accessible: true,
  },
  ...
  {
    fromNodeId: 'escalator_west',
    toNodeId: 'concession_drinks',
    walkTimeSeconds: 20,
    accessible: true,
  },
  {
    fromNodeId: 'concession_drinks',
    toNodeId: 'seating_130',
    walkTimeSeconds: 35,
    accessible: true,
  },
  ...
  {
    fromNodeId: 'elevator_north',
    toNodeId: 'lobby_200_north',
    walkTimeSeconds: 40,
    accessible: true,
  },
  {
    fromNodeId: 'elevator_south',
    toNodeId: 'concession_beers',
    walkTimeSeconds: 55,
    accessible: true,
  },
...
function isNodeAccessible(node: ConcourseNode): boolean {
  ...
  if (node.type === 'restroom') {
    return (
      node.accessibilityTags.includes('rampAccess') ||
      node.accessibilityTags.includes('elevatorAdjacent')
    );
  }
...
export function getGraphData(): {
  nodes: ConcourseNode[];
  edges: ConcourseEdge[];
  accessibleAdj: typeof ACCESSIBLE_ADJ;
} {
  _graphCacheHits++;
  ...
  if (typeof globalThis !== 'undefined' && 'window' in globalThis) {
    ...
    ).window.__matchflowGraphCacheHits = _graphCacheHits;
  }
...
// PathResult type
// ----------------------------------------------------
/** Result of a shortest-path query — either a valid route or an explicit error code. */
